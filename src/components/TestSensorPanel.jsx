...
import { Cpu, Send, CheckCircle2, Car, AlertCircle } from 'lucide-react';
    ...
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-sm mb-8">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="font-bold text-white text-base">
              Sensor Reading Test Control
            </h3>
            <p className="text-xs text-slate-400">
              Module 2 Local Sensor-to-UI Pipeline Test (Click any slot or select below)
            </p>
          </div>
        </div>

        <span className="text-[11px] font-semibold text-amber-400 bg-amber-500/10 border border-amber-500/20 px-2.5 py-1 rounded-full self-start sm:self-auto">
          Dev Pipeline Mode
        </span>
      </div>
      ...
      <div className="mt-4 pt-3 border-t border-slate-800/60 flex items-center justify-between text-xs text-slate-400">
        <div className="flex items-center gap-2">
          ...
          <span>
            Threshold Rule: Distance &lt; 20cm = <strong className="text-rose-400">OCCUPIED</strong>, &ge; 20cm = <strong className="text-emerald-400">AVAILABLE</strong>
          </span>
        </div>
